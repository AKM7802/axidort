...
class Client(UUIDPkMixin, TimestampMixin, Base):
    ...
    (signup/signin, payment, and the client dashboard all operate on this
    one record) — there is no separate multi-user-per-client table.
    """
    ...
    # Role-based auth: ADMIN grants access to /admin/* endpoints, checked
    # directly off this same Client row (see get_current_admin). Not
    # settable via signup or the client-facing update endpoints.
    ...
    # Admin-controlled (not settable via signup): if true, a matching
    # restaurant in this client's territory routes ONLY to this client.
    ...
    do_not_contact_entries: Mapped[list["DoNotContact"]] = relationship(
        back_populates="client", cascade="all, delete-orphan"
    )
