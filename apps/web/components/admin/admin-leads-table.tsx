...
// Admin-only, local copy of the dashboard leads table's column layout —
// components/dashboard/leads-table.tsx is out of scope to edit, so this
// keeps the same UX without touching that file.
...
export function AdminLeadsTableSkeleton() {
  return (
...
export function AdminLeadsTable({
  ...
  if (loading) {
    return <AdminLeadsTableSkeleton />;
  }
        ...
        {leads.map((lead) => {
          ...
          return (
            <TableRow key={lead.id}>
              ...
              <TableCell>{formatDate(event.inspection_date)}</TableCell>
                    ...
                    uniqueCategories(lead).map((category) => {
                      ...
                      return isSubscribed ? (
                        ...
                        <Badge
                          ...
                          title="Also cited on this inspection, outside subscribed categories"
                        >
                          {category}
                        </Badge>
                      );
              ...
              <TableCell>{formatDate(lead.matched_at)}</TableCell>
            </TableRow>
          );
        })}
      </TableBody>
