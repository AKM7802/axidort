...
export function ClientDigestsSection({
  token,
  clientId,
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .adminClientDigests(token, clientId)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    api
      .adminClientDigestDetail(token, clientId, selectedId)
      .then((res) => {
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-1 rounded-lg border border-dashed py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              This client&apos;s first weekly digest will appear here once it&apos;s sent.
            </p>
          </div>
        ) : (
      ...
      <Dialog open={selectedId !== null} onOpenChange={(open) => !open && setSelectedId(null)}>
        <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-3xl">
          ...
          {detailLoading || !detail ? (
            ...
            <AdminLeadsTable leads={detail.leads} loading={false} subscribedCategories={subscribedCategories} />
          )}
        </DialogContent>
      </Dialog>
    </Card>
